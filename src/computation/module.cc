...
Haskell::TopDecls make_topdecls(const CDecls& cdecls)
{
...
void Module::rename(const Program& P)
{
    ...
    if (P.get_module_loader()->dump_renamed and topdecls)
...
void parse_module(const expression_ref& M, string& name, expression_ref& exports, expression_ref& body, expression_ref& impdecls, std::optional<Haskell::TopDecls>& topdecls)
{
    ...
    for(const auto& E: body.sub())
        if (E.is_a<Haskell::TopDecls>())
            topdecls = E.as_<Haskell::TopDecls>();
        else if (is_AST(E,"impdecls"))
            impdecls = E;
}

expression_ref create_module(const string& name, const expression_ref& exports, const expression_ref& impdecls, std::optional<Haskell::TopDecls>& topdecls)
{
...
Haskell::TopDecls rename_top_level(const Haskell::TopDecls& decls, const string& module_name)
{
...
void Module::optimize(const Program& P)
{
    ...
    if (topdecls)
    {
        ...
        topdecls = Haskell::TopDecls(new_decls);
        ...
        if (do_optimize)
        {
            ...
            for(auto& decl: *topdecls)
            {
                assert(is_AST(decl,"Decl"));
                auto& Decl = decl.sub();
                cdecls.push_back({Decl[0].as_<var>(), Decl[1]});
            }
            ...
            cdecls = flatten(std::move(decl_groups));
            topdecls = make_topdecls(cdecls);
